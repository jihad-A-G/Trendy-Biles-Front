...
const EditProduct = () =>{
    const {categories,brands,details,data} = useLoaderData()

    const [selectedCategories, setSelectedCategories] = useState(data.categories);

    const [selectedDetails, setSelectedDetails] = useState(data.details);
    ...
    const handleCategoryChange = (event) => {
       ...
       useEffect(() => {
        ...
        const detailIds = data.details.map(d =>d._id)
        setSelectedDetails(detailIds)
    }, []);
    return(
    <>
    ...
    <Container className='d-flex align-items-center mb-5 p-1 border-bottom border-3'>
            <h3 className='text-black '>Edit product</h3>
        </Container>
  <div className="row">
    <div className="col-md-8 order-md-1">
      ...
      <hr className="mb-4" />
        ...
        {brands.map((b) => (
            ...
            value={b._id}
            name="brand"
            defaultChecked ={b._id === data.brand._id}
          /> {b.name}
            </label>
        </div>
      ))}
      ...
      <input type="hidden" name="formData" value={JSON.stringify(formData)} />
        ...
        <div key={`${d._id}`} className="mb-3">
            ...
            id={`${d.specificName}`}
            value={d._id}
            checked={selectedDetails.includes(d._id)}
        ...
        .then((willDelete) => {
          ...
          submit(e.target.form)
            swal("product was edited successfully!", {
              icon: "success",
            });
